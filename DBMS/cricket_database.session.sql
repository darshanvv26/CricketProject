...
--@block
DELETE FROM player where player_id = 23;
DELETE FROM battingbio where player_id = 23;
DELETE FROM battingHistory where player_id = 23;
DELETE FROM battingStats where player_id = 23;
DELETE FROM bowlingbio where player_id = 23;
DELETE FROM bowlingstats where player_id = 23;
...
--@block
UPDATE bowlingStats AS bs
SET bs.economy = (
    SELECT CASE
        WHEN (bh.balls_bowled_to_left + bh.balls_bowled_to_right) = 0 THEN 0 
        ELSE (((bh.runs_conceded_to_left + bh.runs_conceded_to_right) / 
              ((bh.balls_bowled_to_left + bh.balls_bowled_to_right)/6)))
        END AS result
    FROM bowlingHistory AS bh
    WHERE bh.player_id = bs.player_id 
);
